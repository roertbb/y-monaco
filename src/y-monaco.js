...
import { Selection, SelectionDirection, Range } from "./monaco-editor-files";
            ...
            if (clientID !== this.doc.clientID && state.selection != null && state.selection.anchor != null && state.selection.head != null) {
              ...
              if (anchorAbs !== null && headAbs !== null && anchorAbs.type === ytext && headAbs.type === ytext) {
                ...
                if (anchorAbs.index < headAbs.index) {
                  ...
                  afterContentClassName = `yRemoteSelectionHead ${state.user?.color}`
                  beforeContentClassName = null
                } else {
                  start = monacoModel.getPositionAt(headAbs.index)
                  end = monacoModel.getPositionAt(anchorAbs.index)
                  afterContentClassName = null
                  beforeContentClassName = `yRemoteSelectionHead  ${state.user?.color}`
                }
                newDecorations.push({
                  range: new Range(start.lineNumber, start.column, end.lineNumber, end.column),
                  options: {
                    className: `yRemoteSelection ${state.user?.color}`,
